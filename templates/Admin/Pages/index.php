            ...
            <table class="table table-bordered">
                ...
                <tbody>
                    <?php foreach ($pages as $page): ?>
                        <tr>
                            <td class="text-center align-middle"><?= $this->Form->checkbox('ids[]', ['hiddenField' => false, 'value' => $page->id]) ?></td>
                            <td><?= h($page->name) ?></td>
                            <td><?= h($page->alias) ?></td>
                            <td class="text-center actions">
                                <?= $this->Html->link('<i class="fa-solid fa-eye"></i>', ['action' => 'view', $page->id], ['escape' => false, 'class' => 'btn btn-outline-primary']) ?>
                                <?= $this->Html->link('<i class="fa-solid fa-edit"></i>', ['action' => 'edit', $page->id, '?' => $this->request->getQueryParams()], ['escape' => false, 'class' => 'btn btn-outline-success']) ?>
                                <?=
                                $this->Form->deleteLink('<i class="fa-solid fa-trash"></i>', ['action' => 'delete', $page->id, '?' => $this->request->getQueryParams()],
                                        [
                                            'block' => true,
                                            'escape' => false,
